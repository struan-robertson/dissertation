...
To fairly asses the effect of training a network with slope, an identical network was trained using just the elevation values of the same training data for 500 epochs, referred to as the /No Slope/ model.
...
When tested on this data the /Slope/ model performed better than the /No Slope/ model, with a significant difference in box mask infilling and a smaller difference when infilling split masks to simulate DEM joining.
...
This is a trend when comparing the generated data between networks, despite the fact there is not a significant difference in error on gentle terrain, the data generated by the /Slope/ model is more accurate to the overall structure of the terrain, a metric which is difficult to measure.
...
The /No Slope/ model served as a baseline from which to judge the architecture of the network, proving that it is highly effective at DEM infilling even when trained only on elevation data, however the /Slope/ model has shown that by training a void infilling GAN also on slope values, it can provide the network by encoding data on the overall terrain structure.
...
The other approach to scalability would be implementing a system similar to that proposed by Qui /et al./[cite:@qiuVoidFillingDigital2019], where the large DEM with voids to be infilled would be tiled into 256x256 tiles, input into the network and then infilled results concatenated.
