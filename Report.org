...
To fairly asses the effect of training a network with slope, an identical network was trained using the same training data without slope until it converged, referred to as the /No Slope/ model.
...
| No Slope | Box   | 0.78 |  2.37 |  3.33 | 0.92 |
| No Slope | Split | 0.81 |  2.49 |  3.48 | 0.92 |
...
When tested on this data the /Slope/ and /No Slope/ models perform very similarly, however when using testing data selected manually for complex terrain features (*Appendix x*), the /Slope/ model performs significantly better (Table [[table:complex_terrain]]).
...
The most common artifact in the /Slope/ model is also hallucinated craters, however the difference is that they are located in plausible locations and smaller in scale.
This is a trend when comparing the generated data between networks, despite the fact they have similar rates of error on gentle terrain, the data generated by the /Slope/ model is more accurate to the overall structure of the terrain, a metric which is difficult to measure.
...
| No Slope | Box   | 1.85 | 4.47 | 6.13 | 0.94 |
| No Slope | Split | 1.41 | 3.91 | 5.17 | 0.95 |

The network described by this paper aims to solve two problems with lunar DEMs, void infilling and the joining of two DEMs.
The purpose of the split mask type was to simulate the joining task, so test data generated with the use of the split mask can be used to assess the networks success at DEM joining.
When tested on the randomly sampled terrain both networks performed slightly worse than with a box mask (Table [[table:random_terrain]]), however this is to be expected due to the larger void area.
For the /Slope/ model, the difference between split and mask error types was roughly similar with complex terrain, however the /No Slope/ model was more accurate with split masks (Table [[table:complex_terrain]]).
Visual inspections confirm the fact that the /Slope/ network can join two DEMs with plausible elevation values, as can be seen in Figure \ref{fig:simulated_joining}.
...
Due to a fully convolutional architecture, the network described by this paper can take as input an image of any size and therefor in theory infill a void of any size, the training DEM dimensions of 256 were chosen as most DEM voids should be smaller than this.
...
To pass the first challenge of DEM void infilling proposed by Park and Choi, the network must employ high-resolution reconstruction methodology.
...
The usefulness of SSIM

As gentle terrain appears to be the most common within NAC DEMs, the networks low error rate at infilling voids on terrain like this proves the usefulness of
Unfortunately there no metric for assessing how well
Success at DEM joining

Slope has less artifacts
More accurate to terrain features

The fact slope is better with complex terrain makes a big difference for DEM void infilling, as voids only occur in complex terrain.
