#+title: Lunar Terrain Void Infilling
...
Park and Choi[cite:@parkNeuralProcessApproach2021] authored the most relevant work to this paper, as it also attempts to solve the issue of lunar DEM void reconstruction.
...
The network proposed by Gavriil /et al./[cite:@gavriilVoidFillingDigital2019] shares a similar architecture to the current paper, as both are based on the inpainting network described by Yu /et al./[cite:@yuGenerativeImageInpainting2018].
...
Zhang /et al./[cite:@zhangVoidFillingBased2020] make the argument that the use of local feature extraction by Gavriil /et al./[cite:@gavriilVoidFillingDigital2019] has a negative impact on the networks ability to recover overall DEM semantic information.
...
Dong /et al./[cite:@dongFillingVoidsElevation2020] introduce an additional loss function to train their network to adhere to geometric constraints implied by cast shadows.
...
The terrain texture generation model (TTGM) described by Qiu /et al/.[cite:@qiuVoidFillingDigital2019] served as the inspiration for using slope data in the void infilling network described by the current paper.
...
This paper adapts a modified version of the network architecture described by Zhang /et al./, adding additional convolutional layers to help mitigate artifacts hallucinated by the network.
The use of terrain slope by Qui /et al./ was also adopted to encode terrain structural information such as texture more explicitly.
...
Zhang /et al./[cite:@zhangVoidFillingBased2020] trained without slope using a very similar network architecture and had much more stable reconstruction loss, so it can be inferred that the addition of slope increases the difficulty of network training.
...
A greater difference can be found when the models are tested on data manually selected for complex terrain features (Appendix 4),  where the same pattern repeats (Table [[table:complex_terrain]]).
...
All code for this project can be found in the following github repository [[https://github.com/struan-robertson/DEM-Void-Infilling]].

The accompanying appendix contains:
 - [1] Meeting notes with project supervisor
 - [2] Training logs of No Slope model
 - [3] Training logs of Slope model
 - [4] List of NAC DEMs used in complex terrain testing data
 - [5] List of randomly selected NAC DEMs used for testing data
 - [6] List of DEMs used for training
 - [7] Mid term report.
