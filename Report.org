...
This GAN architecture consists of a two-stage coarse to fine generator (Figure [[fig:generator]]) which increases the receptive fields of the convolutional layers and two discriminators (Figure [[fig:discriminator]]).
...
The convolutional structure for the entire model can be viewed in Table [[table:convolutions]], where the discriminator convolutions are also listed.
...
plt.scatter(epoch, wgan_d, label='Wasserstein Discriminator Loss')
