...
\section{Background}
\label{sec:orgea9cdea}

\subsection{Digital Elevation Maps}
\label{sec:org19a0fa6}
...
\section{Experiments}
\label{sec:orga0dc0e8}


\subsection{Model Training}
\label{sec:orgea082d5}
...
To fairly asses the effect of training a network with slope, an identical network was trained using the same training data without slope until it converged, referred to as the \emph{No Slope} model.
...
No Slope & Box & 0.78 & 2.37 & 3.33 & 0.92\\[0pt]
No Slope & Split & 0.81 & 2.49 & 3.48 & 0.92\\[0pt]
...
When tested on this data the \emph{Slope} and \emph{No Slope} models perform very similarly, however when using testing data selected manually for complex terrain features (\textbf{Appendix x}), the \emph{Slope} model performs significantly better (Table \ref{table:complex_terrain}).
...
The most common artifact in the \emph{Slope} model is also hallucinated craters, however the difference is that they are located in plausible locations and smaller in scale.
This is a trend when comparing the generated data between networks, despite the fact they have similar rates of error on gentle terrain, the data generated by the \emph{Slope} model is more accurate to the overall structure of the terrain, a metric which is difficult to measure.
An interesting observation is that in complex terrain the \emph{No Slope} model was more accurate with a split mask than a box.
...
No Slope & Box & 1.85 & 4.47 & 6.13 & 0.94\\[0pt]
No Slope & Split & 1.41 & 3.91 & 5.17 & 0.95\\[0pt]
\end{tabular}
\end{table}
...
When tested on the randomly sampled terrain both networks performed slightly worse than with a box mask (Table \ref{table:random_terrain}), however this is to be expected due to the larger void area.
For the \emph{Slope} model, the difference between split and mask error types was roughly similar with complex terrain, however the \emph{No Slope} model was more accurate with split masks (Table \ref{table:complex_terrain}).
Visual inspections confirm the fact that the \emph{Slope} network can join two DEMs with plausible elevation values, as can be seen in Figure \ref{fig:simulated_joining}.

\begin{figure*}
  \centering
  \includegraphics[width=\linewidth]{images/split_training.png}
  \captionsetup{justification=centering} 
  \caption{Split infilling results for elevation value and slope. \\ Top: Masked Image, Middle: Infilling Result,  Bottom: Original DEM}
  \label{fig:twocolumn}
\end{figure*}
...
\begin{figure*}
  ...
  \captionsetup{justification=centering} 
  \caption{Box infilling results for elevation value and slope. \\ Top: Masked Image, Middle: Infilling Result,  Bottom: Original DEM}
  \label{fig:twocolumn}
\end{figure*}
