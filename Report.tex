...
\hypersetup{
 ...
 pdftitle={Lunar Terrain Void Infilling},
 pdfkeywords={},
 pdfsubject={},
...
With the launch of NASAs lunar reconnaissance orbiter (LRO), a large amount of high-resolution digital elevation maps (DEMs) have been constructed from data captured by its narrow-angle cameras (NACs) and lunar orbiter laser altimeter (LOLA).
These DEMs provide a precise topographical model of the lunar surface, however are prone to voids containing no data as a result of terrain features blocking the mapping of small regions.
To resolve this issue, this paper uses a machine learning model to allow for the technique of image inpainting to be used with DEM infilling.
Image inpainting uses pixel data from an image to generate missing data to fill a void.
DEMs are two dimensional arrays of height values which, when normalised are equivalent to single channel (greyscale) images, and so the technique of image inpainting can be easily applied to DEMs.
A deep convolutional wasserstein generative adversarial network with contextual attention and gradient penalty was used for the DEM void infilling.
Additionally the network was trained using the slope values of the training data, which encode terrain features to improve the accuracy of the network.
The proposed method was evaluated on both randomly sampled lunar terrain and terrain manually selected for complexity, demonstrating the suggested approach successfully reconstructs no-data voids with high accuracy.


\end{abstract}

\section{Introduction}
\label{sec:org129ff1e}
...
\section{Background}
\label{sec:orgb0b6b8f}

\subsection{Digital Elevation Maps}
\label{sec:org96990fe}
...
\label{sec:orge80af3f}

Park and Choi\autocite{parkNeuralProcessApproach2021} authored the most relevant work to this paper, as it also attempts to solve the issue of lunar DEM void reconstruction.
...
The network proposed by Gavriil \emph{et al.}\autocite{gavriilVoidFillingDigital2019} shares a similar architecture to the current paper, as both are based on the inpainting network described by Yu \emph{et al.}\autocite{yuGenerativeImageInpainting2018}.
...
In the current paper a technique is employed that is likely more effective in the context of lunar DEMs; small objects on the lunar surface such as boulders and craters are highlighted to the network as the slope at the edges of the object is significantly greater than the surrounding terrain, and are not of a very high density.
...
Zhang \emph{et al.}\autocite{zhangVoidFillingBased2020} make the argument that the use of local feature extraction by Gavriil \emph{et al.}\autocite{gavriilVoidFillingDigital2019} has a negative impact on the networks ability to recover overall DEM semantic information.
...
Dong \emph{et al.}\autocite{dongFillingVoidsElevation2020} introduce an additional loss function to train their network to adhere to geometric constraints implied by cast shadows.
...
The terrain texture generation model (TTGM) described by Qiu \emph{et al}.\autocite{qiuVoidFillingDigital2019} served as the inspiration for using slope data in the void infilling network described by the current paper.
...
This paper adapts a modified version of the network architecture described by Zhang \emph{et al.}, adding additional convolutional layers to help mitigate artifacts hallucinated by the network.
The use of terrain slope by Qui \emph{et al.} was also adopted to encode terrain structural information such as texture more explicitly.
Relief degree however was not used, as it was found to have little effect whilst being memory expensive.

\section{Methodology}
\label{sec:org516a158}

\subsection{Problem Formation and Notation}
\label{sec:orga05e7f0}
...
\section{Experiments}
\label{sec:org0fe806b}


\subsection{Model Training}
\label{sec:org40e5916}
...
A list of the DEMs used for training can be found at Appendix 6.
...
Zhang \emph{et al.}\autocite{zhangVoidFillingBased2020} trained without slope using a very similar network architecture and had much more stable reconstruction loss, so it can be inferred that the addition of slope increases the difficulty of network training.
...
A greater difference can be found when the models are tested on data manually selected for complex terrain features (Appendix 4),  where the same pattern repeats (Table \ref{table:complex_terrain}).
...
All code for this project can be found in the following github repository \url{https://github.com/struan-robertson/DEM-Void-Infilling.}

The accompanying appendix contains:
\begin{itemize}
\item\relax [1] Meeting notes with project supervisor
\item\relax [2] Training logs of No Slope model
\item\relax [3] Training logs of Slope model
\item\relax [4] List of NAC DEMs used in complex terrain testing data
\item\relax [5] List of randomly selected NAC DEMs used for testing data
\item\relax [6] List of DEMs used for training
\item\relax [7] Mid term report.
\end{itemize}
\end{document}