...
\section{Background}
\label{sec:org627daf1}

\subsection{Digital Elevation Maps}
\label{sec:org978e950}
...
\section{Experiments}
\label{sec:org082c9ee}


\subsection{Model Training}
\label{sec:orgff4c297}
...
The slope output of the network also serves as a useful tool to measure its ability to learn complex patterns; when viewed as an image instead of rendered in 3D subtle artifact in the elevation values may not be noticed, however it is much more apparent when viewing the slope (Figure \ref{fig:box_training}).
...
By training on slope values, the \emph{Slope} model has proved that the accuracy of a void infilling GAN can be significantly improved, simply by including slope data as a channel in the network input and without modifications to network architecture.
...
The intuition for the improvement of network accuracy in terms of terrain texture when slope is included is that it compliments this process, through the use of slope data the CA layer finds better matching patches from which to copy data.
...
More complex terrain, such as the slope of a crater (Figure \ref{fig:complex_terrain}) is much more challenging for the network to infill even with elevation values, however the slope network performed significantly better.
The CA layer extracts background data to copy in 3x3 patches, however for highly irregular terrain such as that shown in Figure \ref{fig:complex_terrain}, these patches are likely not large enough to copy the texture in a plausible way.
An advantage of training using slope is that the texture is represented the same across the DEM no matter the elevation; with elevation values a texture at the bottom of a crater would cause lower activation than the same texture at the top, however as slope is a measure of the gradient of a pixel in relation to its neighbors it would have a uniform texture, which is much easier for the network to infer patterns from.
...
The other approach to scalability would be implementing a system similar to that proposed by Qui \emph{et al.}\autocite{qiuVoidFillingDigital2019}, where a large DEM with voids to be infilled would be tiled into 256x256 tiles, input into the network and then the infilled results concatenated.
...
Unfortunately there is no method for testing if the network generated plausible data that was not accurate to the original DEM, however this is less of an issue for DEM infilling than image inpainting, as generating data highly accurate to the original DEM is very important for the data to be useful in a real world application.
The low error rates in all metrics measured of the network clearly demonstrate that it is reliable, especially when compared to other void infilling GANs (Table \ref{table:random_terrain}).
